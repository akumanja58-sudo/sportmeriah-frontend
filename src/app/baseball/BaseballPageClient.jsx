...
import { MdSportsSoccer, MdSportsBasketball, MdSportsBaseball, MdSportsHockey, MdPlayArrow, MdLiveTv } from 'react-icons/md';
...
export default function BaseballPageClient() {
    ...
    const fetchMatches = async () => {
        try {
            const res = await fetch(`${API_URL}/api/baseball`);
        ...
        <main className="min-h-screen" style={{ backgroundColor: '#0a0c14' }}>
            ...
            <div className="container max-w-7xl mx-auto px-4 py-8">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h1 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-2"><MdSportsBaseball className="text-red-400" /> MLB / Baseball</h1>
                </div>
                <div className="flex flex-col lg:flex-row gap-8">
                    <aside className="w-full lg:w-72 order-2 lg:order-1 flex-shrink-0">
                        <div className="sticky top-32 space-y-5">
                            <div className="rounded-xl p-5" style={{ backgroundColor: '#1a1d27' }}>
                                <h3 className="text-white font-semibold text-xs mb-4 flex items-center gap-2 uppercase tracking-widest"><MdSportsBaseball className="text-red-400" size={14} /> Statistik</h3>
                                <div className="space-y-3">
                                    ...
                                    <div className="flex justify-between text-sm"><span className="text-gray-500 flex items-center gap-1.5"><HiClock size={12} className="text-red-300" /> Upcoming</span><span className="text-red-300 font-bold">{stats.upcoming || 0}</span></div>
                    ...
                    <div className="flex-1 order-1 lg:order-2 space-y-6">
                        {loading ? (
                            <div className="rounded-xl p-12 text-center" style={{ backgroundColor: '#1a1d27' }}><div className="flex justify-center mb-4"><span className="loader"></span></div><p className="text-gray-500 text-sm">Memuat jadwal MLB...</p><style>{`.loader{width:40px;height:40px;border-radius:50%;display:inline-block;border-top:3px solid #fff;border-right:3px solid transparent;box-sizing:border-box;animation:rot 1s linear infinite;position:relative}.loader::after{content:'';box-sizing:border-box;position:absolute;left:0;top:0;width:40px;height:40px;border-radius:50%;border-left:3px solid #f87171;border-bottom:3px solid transparent;animation:rot .5s linear infinite reverse}@keyframes rot{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}`}</style></div>
                        ) : (<>
                            ...
                            {matches.upcoming.length > 0 && (<section><div className="flex items-center gap-3 mb-4"><HiClock size={18} className="text-red-300" /><h2 className="text-white font-bold text-lg">Upcoming</h2><span className="text-xs text-gray-500 px-2.5 py-0.5 rounded-full font-medium" style={{ backgroundColor: '#232733' }}>{matches.upcoming.length}</span></div><div className="space-y-2">{matches.upcoming.map(m => <MatchCard key={m.id} match={m} isLive={false} />)}</div></section>)}
                            {extraChannels.length > 0 && (<section><div className="flex items-center gap-3 mb-4"><MdLiveTv size={18} className="text-red-300" /><h2 className="text-white font-bold text-lg">Extra Channels</h2><span className="text-xs text-gray-500 px-2.5 py-0.5 rounded-full font-medium" style={{ backgroundColor: '#232733' }}>{extraChannels.length}</span></div><div className="space-y-2">{extraChannels.map(ch => <ExtraCard key={ch.id} channel={ch} />)}</div></section>)}
                            ...
                            {matches.live.length === 0 && matches.upcoming.length === 0 && matches.finished.length === 0 && extraChannels.length === 0 && (<div className="rounded-xl p-12 text-center" style={{ backgroundColor: '#1a1d27' }}><MdSportsBaseball size={40} className="text-gray-600 mx-auto mb-4" /><p className="text-gray-400 font-medium">Tidak ada pertandingan MLB saat ini</p></div>)}
                        </>)}
                        <div className="mt-10 pt-8 space-y-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}><h2 className="text-lg font-semibold text-white">Nonton Streaming MLB Baseball Gratis</h2><p className="text-gray-500 text-sm leading-relaxed">Nonton streaming MLB Baseball gratis di SportMeriah. New York Yankees, LA Dodgers, Houston Astros. HD tanpa buffering.</p></div>
                    </div>
                </div>
            </div>
...
function MatchCard({ match, isLive, isFinished = false }) {
    ...
    const matchUrl = hasStream ? `/baseball/${match.stream?.id}?fixture=${match.id}` : '#';
    return (
        <Link href={matchUrl}><div className={`rounded-lg overflow-hidden cursor-pointer group border ${!hasStream ? 'opacity-50' : ''}`} style={{ backgroundColor: '#1a1d27', borderColor: isLive && hasStream ? 'rgba(248,113,113,0.2)' : 'transparent' }} onMouseEnter={(e) => { if (hasStream) e.currentTarget.style.backgroundColor = '#1e2130'; }} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#1a1d27'}>
            <div className="flex justify-between items-center px-4 py-2 text-xs" style={{ backgroundColor: '#151720' }}>
                ...
                <span className="text-gray-500 truncate max-w-[180px] flex items-center gap-1.5"><MdSportsBaseball size={12} className="text-red-400" />{match.league?.name || 'MLB'}</span>
            </div>
            <div className="flex items-center justify-between px-4 py-3 gap-2">
                ...
                <div className="flex-shrink-0 ml-3">{hasStream ? <span className="text-white text-xs font-semibold px-3.5 py-1.5 rounded-md inline-flex items-center gap-1" style={{ backgroundColor: isLive ? '#dc2626' : isFinished ? '#374151' : '#f87171' }}><MdPlayArrow size={14} /> {isLive ? 'Live' : isFinished ? 'Replay' : 'Tonton'}</span> : <span className="text-gray-500 text-xs font-medium px-3 py-1.5 rounded-md" style={{ backgroundColor: '#232733' }}>No Stream</span>}</div>
            </div>
        </div></Link>
    );
}
...
function ExtraCard({ channel }) {
    return (
        <Link href={`/baseball/${channel.id}`}><div className="rounded-lg overflow-hidden cursor-pointer group border border-transparent" style={{ backgroundColor: '#1a1d27' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#1e2130'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#1a1d27'}>
            <div className="flex justify-between items-center px-4 py-2 text-xs" style={{ backgroundColor: '#151720' }}><span className="font-medium text-red-400 flex items-center gap-1.5"><span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse"></span>Live</span><span className="text-gray-500">{channel.category || 'MLB'}</span></div>
            <div className="flex items-center justify-between px-4 py-3 gap-2"><div className="flex items-center gap-3 flex-1 min-w-0"><div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: '#232733' }}><MdSportsBaseball size={14} className="text-red-400" /></div><span className="text-white text-sm font-medium truncate">{channel.cleanName || channel.name}</span></div><span className="text-white text-xs font-semibold px-3.5 py-1.5 rounded-md inline-flex items-center gap-1 flex-shrink-0" style={{ backgroundColor: '#f87171' }}><MdPlayArrow size={14} /> Tonton</span></div>
        </div></Link>
    );
}
