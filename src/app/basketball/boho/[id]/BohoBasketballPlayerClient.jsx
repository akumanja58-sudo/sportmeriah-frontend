...
export default function BohoBasketballPlayerClient({ matchId }) {
    ...
    const fetchMatchDetail = async () => {
        try {
            setError(null);
            const res = await fetch(`${BOHO_API}?action=detail&id=${encodeURIComponent(matchId)}&sport=basketball`);
            ...
            if (data?.data) {
                ...
                if (data.data.sources?.length > 0) {
                    ...
                    if (isLive) setTimeout(() => startStream(mappedSources[0]), 500);
                }

                if (!isLive && matchTime > now) startCountdown(matchTime);
            } else {
    ...
    const startCountdown = (matchTime) => {
        const updateCountdown = () => {
            ...
            if (diff <= 0) { clearInterval(countdownRef.current); fetchMatchDetail(); return; }
            setCountdown({
    ...
    const startStream = (source) => { setIframeUrl(source.proxyUrl); setIsPlaying(true); setActiveServer(source); };
    const refreshStream = () => { if (!iframeUrl) return; const u = iframeUrl; setIframeUrl(null); setTimeout(() => setIframeUrl(u), 100); };
    ...
    const category = match?.category || 'Basketball';
    ...
    const formatKickoff = () => {
        ...
        return date.toLocaleString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' }) + ' WIB';
    };
                ...
                <div className="relative mb-4">
                    {iframeUrl && isPlaying ? (
                        <div className="rounded-xl overflow-hidden aspect-video bg-black relative">
                            <iframe src={iframeUrl} className="w-full h-full absolute inset-0 border-0" allow="autoplay; encrypted-media; fullscreen" allowFullScreen />
                        </div>
                            ...
                            <div className="absolute inset-0 flex flex-col justify-center items-center text-center p-4">
                                <div className="flex items-center justify-center gap-4 sm:gap-8 mb-5">
                                    <div className="text-center">
                                        {homeBadge ? <img src={homeBadge} alt={homeName} className="w-16 h-16 sm:w-20 sm:h-20 object-contain mx-auto mb-2" onError={(e) => e.target.src = 'https://placehold.co/80x80/232733/6b7280?text=T'} /> : <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center mx-auto mb-2" style={{ backgroundColor: '#232733' }}><MdSportsBasketball size={28} className="text-orange-500" /></div>}
                                    ...
                                    <div className="text-center">
                                        {awayBadge ? <img src={awayBadge} alt={awayName} className="w-16 h-16 sm:w-20 sm:h-20 object-contain mx-auto mb-2" onError={(e) => e.target.src = 'https://placehold.co/80x80/232733/6b7280?text=T'} /> : <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center mx-auto mb-2" style={{ backgroundColor: '#232733' }}><MdSportsBasketball size={28} className="text-orange-500" /></div>}
                                ...
                                <button onClick={() => startStream(sources[0])} className="bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-8 rounded-xl text-base transition-all transform hover:scale-105 flex items-center gap-2 shadow-lg">
                                    <MdPlayArrow size={22} /> Tonton Sekarang
                                </button>
                                {sources.length > 1 && <p className="text-gray-600 text-xs mt-3">{sources.length} server tersedia</p>}
                            </div>
                        </div>
                    ) : isUpcoming ? (
                        <div className="rounded-xl w-full overflow-hidden min-h-[350px] sm:min-h-[400px] md:aspect-video relative" style={{ backgroundColor: '#111318' }}>
                            <div className="absolute inset-0 flex flex-col justify-center items-center text-center p-4">
                                ...
                                <div className="flex justify-center gap-3 sm:gap-4 mb-5">
                                    {countdown.days > 0 && <div className="rounded-lg min-w-[50px] sm:min-w-[60px] px-3 py-2 sm:px-4 sm:py-3 text-center" style={{ backgroundColor: '#1a1d27', border: '1px solid rgba(255,255,255,0.06)' }}><span className="text-xl sm:text-2xl font-bold text-white">{String(countdown.days).padStart(2, '0')}</span><p className="text-[10px] sm:text-xs text-gray-600 mt-1">Hari</p></div>}
                                    <div className="rounded-lg min-w-[50px] sm:min-w-[60px] px-3 py-2 sm:px-4 sm:py-3 text-center" style={{ backgroundColor: '#1a1d27', border: '1px solid rgba(255,255,255,0.06)' }}><span className="text-xl sm:text-2xl font-bold text-white">{String(countdown.hours).padStart(2, '0')}</span><p className="text-[10px] sm:text-xs text-gray-600 mt-1">Jam</p></div>
                                    <div className="rounded-lg min-w-[50px] sm:min-w-[60px] px-3 py-2 sm:px-4 sm:py-3 text-center" style={{ backgroundColor: '#1a1d27', border: '1px solid rgba(255,255,255,0.06)' }}><span className="text-xl sm:text-2xl font-bold text-white">{String(countdown.minutes).padStart(2, '0')}</span><p className="text-[10px] sm:text-xs text-gray-600 mt-1">Menit</p></div>
                                    <div className="rounded-lg min-w-[50px] sm:min-w-[60px] px-3 py-2 sm:px-4 sm:py-3 text-center" style={{ backgroundColor: '#1a1d27', border: '1px solid rgba(255,255,255,0.06)' }}><span className="text-xl sm:text-2xl font-bold text-orange-400">{String(countdown.seconds).padStart(2, '0')}</span><p className="text-[10px] sm:text-xs text-gray-600 mt-1">Detik</p></div>
                                </div>
                                <p className="text-gray-600 text-xs mb-3">Stream akan tersedia saat match dimulai</p>
                                <Link href="/basketball" className="text-orange-400 hover:text-orange-300 text-sm font-medium flex items-center gap-1 transition-colors"><MdArrowBack size={16} /> Kembali ke Basketball</Link>
                            </div>
                        </div>
                    ) : (
                        ...
                        <div className="rounded-xl w-full overflow-hidden min-h-[350px] sm:min-h-[400px] md:aspect-video relative" style={{ backgroundColor: '#111318' }}>
                            <div className="absolute inset-0 flex flex-col justify-center items-center text-center p-4">
                                <div className="flex items-center justify-center gap-4 sm:gap-8 mb-5">
                                    <div className="text-center">
                                        {homeBadge ? <img src={homeBadge} alt={homeName} className="w-14 h-14 sm:w-16 sm:h-16 object-contain mx-auto mb-2 opacity-50" onError={(e) => e.target.src = 'https://placehold.co/64x64/232733/6b7280?text=T'} /> : <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center mx-auto mb-2 opacity-50" style={{ backgroundColor: '#232733' }}><MdSportsBasketball size={24} className="text-gray-600" /></div>}
                                    ...
                                    <div className="text-center">
                                        {awayBadge ? <img src={awayBadge} alt={awayName} className="w-14 h-14 sm:w-16 sm:h-16 object-contain mx-auto mb-2 opacity-50" onError={(e) => e.target.src = 'https://placehold.co/64x64/232733/6b7280?text=T'} /> : <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center mx-auto mb-2 opacity-50" style={{ backgroundColor: '#232733' }}><MdSportsBasketball size={24} className="text-gray-600" /></div>}
                ...
                {isPlaying && sources.length > 0 && (
                    <div className="mb-4 rounded-xl p-3 sm:p-4" style={{ backgroundColor: '#1a1d27' }}>
                        <div className="flex items-center gap-2 mb-3">
                            <HiSignal size={14} className="text-orange-400" />
                        ...
                        <div className="flex flex-wrap gap-2">
                            {sources.map((src) => {
                                ...
                                return (
                                    <button key={`srv-${src.streamNo}`} onClick={() => startStream(src)}
                                        className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${isActive ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/20' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                ...
                <div className="flex flex-col lg:flex-row gap-5">
                    <div className="w-full lg:w-3/4 space-y-4">
                        ...
                        <div className="rounded-xl p-4" style={{ backgroundColor: '#1a1d27' }}>
                            <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="text-gray-500 text-xs sm:text-sm flex items-center gap-2"><MdSportsBasketball className="text-orange-500 flex-shrink-0" size={14} /><span className="truncate">{category}</span></p>
                                </div>
                                {isLive && (
                        ...
                        <div className="rounded-xl p-4 hidden sm:block" style={{ backgroundColor: '#1a1d27' }}>
                            <h2 className="text-base font-semibold text-white mb-2">Streaming Basketball Gratis</h2>
                            <p className="text-gray-500 text-sm leading-relaxed">Tonton {matchTitle} secara gratis di NobarMeriah. Live streaming basketball dengan kualitas HD, multi server, tanpa buffering.</p>
                        </div>
                    </div>
                    ...
                    <div className="w-full lg:w-1/4">
                        <div className="rounded-xl p-4 lg:sticky lg:top-32" style={{ backgroundColor: '#1a1d27' }}>
                            ...
                            <div className="space-y-2">
                                <Link href="/basketball" className="block text-gray-500 hover:text-orange-400 text-sm flex items-center gap-2 transition-colors"><MdSportsBasketball size={14} /> Semua Pertandingan</Link>
